...
T5Registers::T5Registers() {
	...
	T5Registers::moduleNumber = 255;	//Default value of T5Register, if moduleNumber is 255 then the number has not been set. setModuleNumber = false
	T5Registers::modNumberSet = false;

	//T5Registers::getModuleNumber();
...
uint8_t T5Registers::getModuleNumber(){
	return this->moduleNumber;

	/*	if(this->modNumberSet == true && this->moduleNumber != 255){	//if modnumset true and modnumber
		return this->moduleNumber;
	}
	else{
...
bool T5Registers::setModuleNumber(uint8_t modNum){
	if(this->getModuleNumber() == 255){
		this->moduleNumber = modNum;
		this->modNumberSet = true;
		return true;
	}//if
	else if(this->modNumberSet){
		return false;
	}//else if
	else{
		return true;
...
T5Registers::T5Registers(const T5Registers &other) {
	// TODO Auto-generated constructor stub
	if(!other.modNumberSet){
		// TODO Create ERROR LOG
	}//if module number not set log error
	else if(other.moduleNumber == 255){
		// TODO Create Error log
	}
	else{
		this->detectorID = other.detectorID;
		this->fifoStatistics = other.fifoStatistics;
		this->fifoStatusASIC0 = other.fifoStatusASIC0;
		this->fifoStatusASIC1  = other.fifoStatusASIC1;
		this->fifoStatusASIC2 = other.fifoStatusASIC2;
		this->fifoStatusASIC3 = other.fifoStatusASIC3;
		this->fpgaVersion = other.fpgaVersion;
		this->latchedFIFOStatusASIC0 = other.latchedFIFOStatusASIC0;
		this->latchedFIFOStatusASIC1 = other.latchedFIFOStatusASIC1;
		this->latchedFIFOStatusASIC2 = other.latchedFIFOStatusASIC2;
		this->latchedFIFOStatusASIC3 = other.latchedFIFOStatusASIC3;
		this->latchedStatus = other.latchedStatus;
		this->modNumberSet = other.modNumberSet;
		this->moduleNumber = other.moduleNumber;
		this->packetStatistics = other.packetStatistics;
		this->rampCountStatistics = other.rampCountStatistics;
		this->serialIDLSW = other.serialIDLSW;
		this->serialIDMSW = other.serialIDMSW;
		this->status = other.status;
		this->tackStatistics = other.tackStatistics;
		this->triggerFIFOStatus = other.triggerFIFOStatus;
		this->triggerStatistics = other.triggerStatistics;
		//loop for copying all registers
		for(int i=0; i < 84;i++){
...
// Copy Constructor
T5Registers::T5Registers(const T5Registers &other) {
    modNumberSet = other.modNumberSet;
    moduleNumber = other.moduleNumber;
    for (int i = 0; i < 84; i++) {
        registers[i] = other.registers[i];
    }//for

/*    fpgaVersion.bits = other.fpgaVersion.bits;
    fpgaVersion.function = other.fpgaVersion.function;
    fpgaVersion.RW = other.fpgaVersion.RW;
    fpgaVersion.defaultValue = other.fpgaVersion.defaultValue;*/
}//T5Registers(&other)
...
/*    fpgaVersion.bits = other.fpgaVersion.bits;
    fpgaVersion.function = other.fpgaVersion.function;  // std::string will use its move assignment
    fpgaVersion.RW = other.fpgaVersion.RW;              // same here
    fpgaVersion.defaultValue = other.fpgaVersion.defaultValue;*/
}
